...
class DetailViewController: UIViewController, CollectionViewControllerDelegate, UIScrollViewDelegate {
    ...
    @objc var isDark = true {
        didSet {
            guard isDark != oldValue && UIDevice.current.userInterfaceIdiom == .tv else {
                return
            }

            children.forEach {
                guard $0.responds(to: #selector(getter:DetailViewController.isDark)) else {
                    return
                }
    ...
    override dynamic func viewDidLoad() {
        ...
        if let image = currentItem.largeBackgroundImage, let url = URL(string: image) {
            backgroundImageView.af.setImage(withURL: url) { [weak self] response in
                guard let image = response.value, let `self` = self else {
                    return
                }
        ...
        let completion: (String?, NSError?) -> Void = { [weak self] (image, error) in
            guard let image = image, let url = URL(string: image), let `self` = self else {
                return
            }
