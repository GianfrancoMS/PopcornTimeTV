...
public final class UpdateManager: NSObject {
    ...
    private var skipReleaseVersion: VersionString? {
        get {
            guard let data = UserDefaults.standard.data(forKey: "skipReleaseVersion") else {
                return nil
            }
    ...
    private func checkForUpdates(_ completion: ((_ success: Bool) -> Void)? = nil) {
        ...
        AF.request("https://api.github.com/repos/PopcornTimeTV/PopcornTimeTV/releases").validate().responseJSON { (response) in
...
internal class VersionString: NSObject, NSCoding {
    ...
    required init?(coder aDecoder: NSCoder) {
        guard let date = aDecoder.decodeObject(forKey: "date") as? Date,
              ...
              let releaseType = ReleaseType(rawValue: releaseTypeRawValue) else {
            return nil
        }
        self.date = date
